...
module.exports = __webpack_require__(143)(exports);
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(196)(module), __webpack_require__(2)))
...
var inherits = __webpack_require__(1)
  ...
  , SenderReceiver = __webpack_require__(64)
  ;
...
var inherits = __webpack_require__(1)
  ...
  , objectUtils = __webpack_require__(45)
  ;
...
var inherits = __webpack_require__(1)
  ...
  , InfoAjax = __webpack_require__(56)
  ;
...
var inherits = __webpack_require__(1)
  ...
  , EventSourceReceiver = __webpack_require__(163)
...
var inherits = __webpack_require__(1)
  , HtmlfileReceiver = __webpack_require__(164)
  , XHRLocalObject = __webpack_require__(19)
  , AjaxBasedTransport = __webpack_require__(14)
  ;
...
var inherits = __webpack_require__(1)
  , urlUtils = __webpack_require__(8)
  , BufferedSender = __webpack_require__(161)
  , Polling = __webpack_require__(162)
  ;
...
var required = __webpack_require__(148)
...
exports.interceptor = exports.timeoutHandler = exports.wsClient = undefined;

var _ws = __webpack_require__(125);

var _ws2 = _interopRequireDefault(_ws);

var _websocket = __webpack_require__(127);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

// /**
//  * 文件用途说明 : WEBSOCKET入口文件，默认设置了WEBSOCKET连接器实例
//  *
//  * 项目名称 : dfzq-obboot-mobi
//  * 作者姓名 : crixusshen
//  * 创建日期 : 16/3/9
...
var wsClient = function wsClient(_) {
    _ws2.default.setPlatformDefaultClient(_websocket.websocket);
    return _ws2.default;
};
...
(function (define) {
    ...
    !(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
        ...
        var SockJS = __webpack_require__(149);
...
/**
 * WebSocket连接器 具体实现 （实现了重连机制）
 * Created by dfzq on 2017/8/23.
 */
(function (define, global) {
    ...
    !(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
        ...
        var responsePromise, client, eventBus;

        client = __webpack_require__(12);
        responsePromise = __webpack_require__(18);
        eventBus = __webpack_require__(126);

        /// Private state variables

        // // EventBus实例，基于sock.js和vert.x (修改为连接池模式，因此这里被取消了)
        // var eb = null
        // 待发送和注册Handler
        var toBeHandlers = [];
        // EventBus连接池
         ...
         * 建立websocket通讯层
         */
        function setupEventBus(request) {
            // 引入websocket连接池思想
            var existEbInstance = ebPool.find(function (ele) {
                return ele.connectionPath.indexOf(request.path) != -1;
            });

            var eb = void 0;
            if (!existEbInstance) {
                eb = new eventBus(request.path);

                // 将新创建的eb实例放入连接池内
                var ebInstance = {
                    connectionPath: request.path,
                    ebInstance: eb,
                    // 重连次数
                    reconnectAttempt: 0,
                    // 最大重连次数
                    maxReconnectAttempt: null,
                    // 是否超时标识符
                    timedOut: false,
                    // 当前连接状态
                    readyState: WebSocket.CONNECTING
                };
                ebPool.push(ebInstance);
                ...
                var timeout = setTimeout(function () {
                    ebInstance.timedOut = true;
                    localWs.close();
                    ebInstance.timedOut = false;
                }, timeoutInterval);

                eb.onopen = function () {
                    // 清除超时处理器
                    clearTimeout(timeout);
                    console.info('WebSocket[' + eb.sockJSConn.url + '][onopen]');

                    // 重置相关属性
                    ebInstance.readyState = WebSocket.OPEN;
                    ebInstance.reconnectAttempt = 0;

                    // 一旦链接被成功建立后，发送暂存toBeHandlers
                    if (toBeHandlers.length > 0) {

                        toBeHandlers.forEach(function (handler) {
                            if (eb.sockJSConn.url.indexOf(handler.gateway) != -1 && !handler.fulfill) {
                                if (handler.type == 'send') {
                                    eb.send(handler.channel, handler.body, handler.headers, handler.replyHandler);
                                    handler.fulfill = true;
                                }

                                // 实现双工回调的函数注册
                                if (handler.type == 'register') {
                                    eb.registerHandler(handler.channel, handler.headers, handler.replyHandler);
                                    handler.fulfill = true;
                                }
                            }
                        });
                ...
                // 重发机制
                eb.onclose = function () {
                    console.info('WebSocket[' + eb.sockJSConn.url + '][onclose]');
                    var existEbInstanceIndex = ebPool.findIndex(function (ele) {
                        return ele.connectionPath.indexOf(request.path) != -1;
                    });
                    // 找到了对应的索引值，进行删除
                    if (existEbInstanceIndex != -1) {
                        ebPool.splice(existEbInstanceIndex, 1);
                    }
                    ...
                    // 设置连接实例的属性
                    ebInstance.readyState = WebSocket.CONNECTING;

                    // 根据递减算法计算重连连接的延时增长时间
                    var timeout = reconnectInterval * Math.pow(reconnectDecay, ebInstance.reconnectAttempt);
                    setTimeout(function () {
                        ebInstance.reconnectAttempt++;
                        setupEventBus(request);
                    }, timeout > maxReconnectInterval ? maxReconnectInterval : timeout);
                };
            } else {
                eb = existEbInstance.ebInstance;
            }

            return eb;
        }

        var websocketClient = client(function websocket(request) {
            return responsePromise.promise(function (resolve, reject) {

                // 去除url上的get和\参数
                if (request.path.indexOf('?') != -1) {
                    request.path = request.path.substring(0, request.path.indexOf('?'));
                }
                request.path = request.path.replace(/\/ /, '').trim();

                // 建立ws通道
                var eb = setupEventBus(request);
                ...
                // 消息体JSON对象结构
                var msgBody = JSON.parse(request.entity);

                // 当websocket正在建立链接时，有toBeSendHandler进行注册则进行暂存，待链接被open时才进行调用
                if (eb.state == eventBus.CONNECTING) {
                    // 暂存Handler中的待发送消息体结构
                    var envelope = {
                        gateway: request.path, // 对应的网关地址
                        type: 'send',
                    ...
                    toBeHandlers.push(envelope);
                } else {
                    eb.send(request.channel, msgBody, headers, callback);
                }

                function callback(err, rec) {
                    // 框架级别异常处理
                    if (err) {
                        // 通讯层发生超时
                        if (err.failureType == 'TIMEOUT') {
                            resolve({
                                entity: {
                                    code: 7100
                                }
                        ...
                        response.address = rec.address;
                        response.type = rec.type;
                        response.entity = rec.body;
                        resolve(response);
                    }
                }
            });
         ...
         * @param impl 当前连接实例
         */
        var mixedEb = function mixedEb(impl) {
             ...
             * @param callback  回调函数
             */
            impl.register = function (address, headers, callback) {
                ...
                if (impl.state == eventBus.CONNECTING) {
                    // 暂存Handler中的待发送消息体结构
                    var envelope = {
                        gateway: impl.sockJSConn.url,
                        type: 'register',
                        channel: address,
                        headers: headers,
                        body: null,
                    ...
                    impl.registerHandler(address, headers, callback);
                }
            };
             ...
             * @param callback  回调函数
             */
            impl.unregister = function (address, headers, callback) {
                impl.unregisterHandler(address, headers, callback);
            };
        ...
        /**
         * 从连接池中获取对应网关的连接实例
         * 如果连接池中没有则会自动构建，如果存在则直接取该已创建的连接实例
         * @param path  网关绝对路径
         * @returns {Array}
         */
        websocketClient.getInstanceFromConnectionPool = function (path) {
            if (!path && !(path.startsWith('http:') || path.startsWith('https:'))) {
                throw new Error('param 0 of getInstanceFromConnectionPool is required.');
            }
            ...
            var existEbInstance = ebPool.find(function (ele) {
                return ele.connectionPath.indexOf(path) != -1;
            });

            var currentEb = void 0;
            if (!existEbInstance) {
                // 自动构建该连接实例
                currentEb = setupEventBus({
                    path: path
                });
            } else {
                // 当前连接实例
                currentEb = existEbInstance.ebInstance;
            }

            return mixedEb(currentEb);
        };
...
var transportList = __webpack_require__(158);

module.exports = __webpack_require__(156)(transportList);
...
var urlUtils = __webpack_require__(8)
  , eventUtils = __webpack_require__(13)
  , JSON3 = __webpack_require__(10)
  , FacadeJS = __webpack_require__(152)
...
var EventEmitter = __webpack_require__(5).EventEmitter
  ...
  , XHRFake = __webpack_require__(167)
  , InfoIframe = __webpack_require__(154)
  , InfoAjax = __webpack_require__(56)
  ;
...
var URL = __webpack_require__(72)
  ...
  , JSON3 = __webpack_require__(10)
  , random = __webpack_require__(15)
  , escape = __webpack_require__(172)
  , urlUtils = __webpack_require__(8)
  , eventUtils = __webpack_require__(13)
  , transport = __webpack_require__(174)
  , objectUtils = __webpack_require__(45)
  , browser = __webpack_require__(20)
  , log = __webpack_require__(173)
  , Event = __webpack_require__(42)
  , EventTarget = __webpack_require__(55)
  , loc = __webpack_require__(58)
  , CloseEvent = __webpack_require__(150)
  , TransportMessageEvent = __webpack_require__(151)
  , InfoReceiver = __webpack_require__(155)
  ;
...
module.exports = function(availableTransports) {
  ...
  __webpack_require__(153)(SockJS, availableTransports);
  return SockJS;
};
...
, __webpack_require__(43)(__webpack_require__(61))
...
, __webpack_require__(43)(__webpack_require__(66))
...
var inherits = __webpack_require__(1)
  , SenderReceiver = __webpack_require__(64)
  , JsonpReceiver = __webpack_require__(165)
  , jsonpSender = __webpack_require__(166)
  ;
...
var utils = __webpack_require__(13)
  ...
  , inherits = __webpack_require__(1)
  , EventEmitter = __webpack_require__(5).EventEmitter
  , WebsocketDriver = __webpack_require__(159)
  ;
